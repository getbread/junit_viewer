...
function replaceClass(element, fromClassName, toClassName) {
  if (element) element.className = element.className.replace(fromClassName, toClassName);
}
...
function toggleContraction(element) {
  ...
  if (!~element.className.indexOf('framed') && ~element.children[0].className.indexOf('failure')) {
    try {
      ...
      let url = suiteButton.textContent.split(' at url ')[1].split(' ')[0] + '#bread-checkout-btn-product';
      frame.src = url;
...
function createVisibleInfo(suites) {
  ...
  Object.keys(suites)
    ...
    .forEach(function(suiteKey, index) {
      var suite = document.getElementById(suiteKey);
      if (
        suite.className.indexOf('not_in_search') === -1 &&
        ...
        if (!data.suites.hasOwnProperty(type)) data.suites[type] = 0;
        data.suites[type] += 1;
        var suiteContents = suite.children[1].children;
        Object.keys(suiteContents)
          .filter(function(testKey) {
            return (
              document.getElementById(testKey) !== null &&
          ...
          .forEach(function(testKey) {
            ...
            if (!data.tests.hasOwnProperty(type)) data.tests[type] = 0;
            data.tests[type] += 1;
          });
      }
    });
...
function updateInfo() {
  ...
  data.suites.count =
    suites.length - hiddenSuites.length - notInSearchSuites.length - noVisibleTestsSuites.length;
  ...
  Object.keys(junit_info.suites).forEach(function(key) {
    ...
    else document.getElementById('junit_info_suite_' + key + '_count').innerHTML = 0;
  });
  ...
  Object.keys(junit_info.tests).forEach(function(key) {
    ...
    else document.getElementById('junit_info_test_' + key + '_count').innerHTML = 0;
  });
}
...
function hidePassingSuites(button) {
  ...
  suites.forEach(function(suite) {
    var suiteElement = document.getElementById(suite.id);
    if (isHidden) removeClass(suiteElement, 'hidden');
    else if (suite.type === 'passed') addClass(suiteElement, 'hidden');
  });
...
function searchSuites(value) {
  ...
  suites.forEach(function(suite) {
    ...
    if (!inSearch && notAlreadySearched) addClass(suiteElement, 'not_in_search');
    if (inSearch) removeClass(suiteElement, 'not_in_search');
  });
...
function hideTests(button) {
  ...
  forEachTest(function(test) {
    var testElement = document.getElementById(test.id);
    if (isHidden) removeClass(testElement, 'hidden');
    else addClass(testElement, 'hidden');
  });
...
function hidePassingTests(button) {
  ...
  forEachTest(function(test) {
    var testElement = document.getElementById(test.id);
    if (isHidden) removeClass(testElement, 'hidden');
    else if (test.type === 'passed') addClass(testElement, 'hidden');
  });
...
function searchTests(value) {
  ...
  forEachTest(function(test) {
    ...
    test.messages.values.forEach(function(message) {
      ...
      if (isInMessage) inSearch = true;
    });

    var notAlreadySearched = testElement.className.indexOf('not_in_search') === -1;
    if (!inSearch && notAlreadySearched) addClass(testElement, 'not_in_search');
    if (inSearch) removeClass(testElement, 'not_in_search');
  });
  ...
  suites.forEach(function(suite) {
    ...
    var numberOfHiddenTests = Object.keys(testElements).filter(function(key) {
      return (
        ...
        (testElements[key].className.indexOf('not_in_search') !== -1 ||
    ...
    else suiteElement.className = suiteElement.className.replace(' no_visible_tests', '');
  });
...
function hideProperties(button) {
  ...
  suites
    ...
    .forEach(function(suite) {
      var propertiesElement = document.getElementById(suite.properties.id);
      if (isHidden) removeClass(propertiesElement, 'hidden');
      else addClass(propertiesElement, 'hidden');
    });
}
...
function searchProperties(value) {
  ...
  suites.forEach(function(suite) {
    suite.properties.values.forEach(function(property) {
      ...
      if (!inSearch && notAlreadySearched) addClass(propertyElement, 'not_in_search');
      if (inSearch) removeClass(propertyElement, 'not_in_search');
    });
  });
}
